...
for eachField, i in zip(fields, range(len(fieldnames))):
    ...
    aecv[aecv < -5000] = np.nan
    print 'Differences of random stations: ', aecv
    print 'Mean error of random stations', aecv.mean()

    rmse = np.sqrt((abs(aecv) ** 2).mean()).values.tolist()
    #rmse.squeeze()
    ...
    #RMSE.to_csv(r'I:\DOCUMENTS\WEGC\02_PhD_research\04_Programming\QGIS\Interpolation\CrossVal\CV_differences_per_run\RMSE_CV.csv')
    np.savetxt(path2dest3 + '/RMSE.csv', RMSE, delimiter=',')

# in this table, there is one column for each extreme day, holding 10 rows with the 10 interpolated values