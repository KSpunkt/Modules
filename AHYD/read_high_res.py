...
src = r'I:\DOCUMENTS\WEGC\02_PhD_research\03_Data\AHYD\High_resolution'

emptylist = []
Stations = {'Steiermark': None, 'Tirol': None, 'Salzburg': None,
            'Burgenland': None, 'Kaernten': None, 'Niederoesterreich': None}


for county in Stations.keys():
    '''Create a dictionary of synnr and station name from csv'''
    with open(str(src) + '/AHYD_'+ county +'\Stations_highres_'+ county +'.csv') as stationcsvin:
        reader = csv.reader(stationcsvin)
        stat_dict = {rows[1]: rows[0] for rows in reader}
        stat_dict.pop('name', None)
        Stations[county] = stat_dict

for eachStatNr in Stations['Tirol'].values():
